...
1) s0 <- {q0}; K <- [{q0}] - множество вершин для детерминизации (очередь, так как необходимо удалять ДВ и добавлять новые для детерминирования, очень походе на очередь. Однако необходимо так же и быстро проверять наличие элемента в этом множестве, а за линейку делать этого не хочется. Так что возможно и дерево. Пока будем счиать это очередью)(или вообще хеш-таблицей)(да, это теперь хеш-таблица). D <- [] - множество детерминированных вершин, тоже хеш-таблица.
2) Пока K не пусто, s <- K.достать_любой(), детерминировать s, добавить s в множество детерминированных вершин.
...
1) Для каждого входного символа `ai`: `si <- t(qi1,ai) \/ t(qi2,ai) \/ ... \/ t(qip,ai)`
